...
export default function ChallengeDayPage() {
  ...
  const proofMissing = searchParams.get("proof") === "missing";
  ...
  return <AppShell><div className="page space-y-8">
    ...
    <section aria-labelledby="proof-heading"><div className="flex items-end justify-between"><div><SectionLabel>Your proof</SectionLabel><h2 id="proof-heading" className="display mt-2 text-2xl">{proofMissing ? "One proof left." : "Ready for verification."}</h2></div><StatusIndicator status={proofMissing ? "pending" : "verified"} label={proofMissing ? "1 / 2" : "2 / 2"} /></div><div className="mt-4 divide-y divide-line border-y border-line"><div className="py-4"><div className="flex items-center justify-between"><div className="flex items-center gap-2"><Github size={17} className="text-signal" /><p className="font-mono text-xs font-semibold uppercase tracking-[0.09em]">GitHub repository</p></div><StatusIndicator label="connected" /></div><p className="mt-3 truncate font-mono text-xs text-muted">github.com/arjun/day-12-api</p></div><div className="py-4"><div className="flex items-center justify-between"><div className="flex items-center gap-2"><Linkedin size={17} className={proofMissing ? "text-muted" : "text-signal"} /><p className="font-mono text-xs font-semibold uppercase tracking-[0.09em]">LinkedIn post</p></div><StatusIndicator status={proofMissing ? "pending" : "verified"} label={proofMissing ? "missing" : "posted"} /></div><p className="mt-3 truncate font-mono text-xs text-muted">{proofMissing ? "Share today’s learning note to complete proof." : "linkedin.com/posts/arjun/day-12"}</p></div></div>{proofMissing ? <p className="mt-3 border-l border-amber-200/50 pl-3 text-xs leading-5 text-muted">Your build is connected. Add the LinkedIn post when you&apos;re ready to complete the public proof chain.</p> : <p className="mt-3 font-mono text-[10px] uppercase tracking-eyebrow text-signal">✓ Build proof + learning proof verified</p>}</section>

    {complete ? <CompletionState /> : <Button disabled={proofMissing} onClick={() => setComplete(true)} className="w-full" size="lg">{proofMissing ? "LinkedIn proof required" : "Complete day 12"} <CheckCircle2 size={17} /></Button>}
  </div></AppShell>;
}
