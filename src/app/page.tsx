  ...
  return <AppShell><div className="page space-y-14 sm:space-y-20">
    ...
    <section aria-labelledby="timeline-heading" className="border-y border-line py-5">
      ...
      <div className="mt-7 overflow-hidden border border-line bg-[#0d0e0c] p-3">
        <div className="flex items-center gap-2 text-[10px] font-medium uppercase tracking-eyebrow text-muted"><span className="size-1.5 bg-signal" /> build sequence <span className="ml-auto font-mono text-signal">protocol</span></div>
        <div className="relative mt-5 flex items-center justify-between before:absolute before:left-3 before:right-3 before:top-5 before:h-px before:bg-line">
          {timelineDays.map((day) => <ChallengeDayIndicator key={day} day={day} status={day === 1 ? "current" : "locked"} />)}
    ...
    <section className="border-t border-line pt-7">
      ...
      <Link href="/dashboard?scenario=first-day" className="mt-7 flex h-12 items-center justify-between border border-signal/45 px-4 font-mono text-xs font-semibold uppercase tracking-[0.1em] text-signal transition-colors hover:bg-signal hover:text-graphite">Begin day 01 <ArrowUpRight size={16} /></Link>
    </section>
  </div></AppShell>;
}
